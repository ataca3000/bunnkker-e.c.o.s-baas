...
import { BunkkerTunnel, getLicenseStatus } from '@bunkker/core';
...
// Iniciar Túnel Inverso (si la licencia está activa)
async function initTunnel() {
    const license = getLicenseStatus();
    if (!license.isActive) {
        console.warn("⛔ [Worker] Licencia inactiva/expirada. El túnel no se iniciará.");
        return;
    }

    const tenantId = 'demo-hardware'; // Aquí idealmente leeríamos el config del tenant

    const tunnel = new BunkkerTunnel({
        port: 3000,
        subdomain: `bunkker-${tenantId}`,
        onUrlGenerated: async (url) => {
            try {
                ...
                console.log(`📡 [Worker] URL del túnel registrada en la nube: ${url}`);
            } catch (err) {
                console.error("Error registrando URL en Firebase:", err);
            }
        }
    });
