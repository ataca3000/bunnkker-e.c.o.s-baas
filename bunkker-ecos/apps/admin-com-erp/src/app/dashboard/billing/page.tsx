...
import { db, auth } from '@bunkker/core';
...
function FacturacionContent() {
            ...
            const verifyPayment = async () => {
                ...
                try {
                    ...
                    if (data.status === 'paid') {
                        ...
                        alert("✅ ¡Pago Confirmado! Tus créditos han sido abonados.");
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        if (!orderFound || !orderFound.items || orderFound.items.length === 0) {
            alert("No hay productos válidos para facturar en esta orden.");
            return;
        }
    ...
    const handleBuyCredits = (monto: number) => {
        ...
        alert(`🔔 Instrucciones para activar tus créditos:\n\n1. Realiza el pago de $${monto} MXN en PayPal.\n2. Envía el comprobante al Desarrollador por WhatsApp.\n3. Tus créditos se reflejarán en minutos.\n\n(Se abrirá WhatsApp para que envíes el comprobante ahora)`);
        setTimeout(() => {
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                <header style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <div style={{ background: '#E8F5E9', padding: '10px 15px', borderRadius: '8px', marginTop: '10px', borderLeft: '4px solid #27ae60', fontSize: '0.85rem', color: '#1b5e20' }}>
                    ...
                    <div className="card-sanjose bg-slate-800/80 shadow-lg rounded-xl" style={{ padding: '0.8rem 1.5rem', borderLeft: '4px solid #FFCB05', display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <div style={{ backgroundColor: '#FFF8E1', padding: '8px', borderRadius: '50%' }}>
                            <Coins color="#F57F17" size={24} />
                        </div>
                        <div>
                            <span style={{ fontSize: '0.7rem', color: '#666', fontWeight: 'bold', display: 'block' }}>SALDO DISPONIBLE</span>
                            <span style={{ fontSize: '1.2rem', fontWeight: '900', color: '#333' }}>{ownerCredits} Facturas</span>
                        </div>
                        <button
                    ...
                    <div className="card-sanjose bg-slate-800/80 p-6 rounded-2xl shadow-lg border border-slate-700/50">
                        ...
                        {success ? (
                            ...
                            <div className="space-y-6">
                                {/* Búsqueda de Orden */}
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-700/50">
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            className="flex-1 p-3 border border-slate-600/50 rounded-lg outline-none focus:border-sky-500" 
                                        />
                                        <button 
                                ...
                                {orderFound && (
                                    <motion.form 
                                        ...
                                        className="space-y-5"
                                    >
                                        <div className="p-4 border border-sky-100 bg-sky-50 rounded-xl flex justify-between items-center">
                                            <div>
                                                <div className="text-xs font-bold text-sky-600 uppercase">Orden Encontrada</div>
                                                <div className="font-bold text-white">{orderFound.id} ({orderFound.items?.length} items)</div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-xs font-bold text-sky-600 uppercase">Monto Total</div>
                                                <div className="font-black text-lg text-sky-700">{formatCurrency(orderFound.total)}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input required type="text" placeholder="Ej. PUBLICO EN GENERAL" value={legalName} onChange={e => setLegalName(e.target.value.toUpperCase())} className="w-full p-3 border border-slate-600/50 rounded-lg outline-none focus:border-sky-500 uppercase" />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">RFC *</label>
                                                <input required type="text" placeholder="Ej. XAXX010101000" value={rfc} onChange={e => setRfc(e.target.value.toUpperCase().replace(/\s/g, ''))} className="w-full p-3 border border-slate-600/50 rounded-lg outline-none focus:border-sky-500 uppercase" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input required type="text" maxLength={5} placeholder="00000" value={zipCode} onChange={e => setZipCode(e.target.value.replace(/\D/g, ''))} className="w-full p-3 border border-slate-600/50 rounded-lg outline-none focus:border-sky-500" />
                                            </div>
                                            <div>
                                        ...
                                        <div>
                                            ...
                                            <input type="email" placeholder="Para enviar la factura..." value={customerEmail} onChange={e => setCustomerEmail(e.target.value)} className="w-full p-3 border border-slate-600/50 rounded-lg outline-none focus:border-sky-500" />
                                        </div>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="card-sanjose bg-slate-800/80 p-6 rounded-2xl shadow-lg border border-slate-700/50">
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-700/50">
                                    <span className="text-sm text-slate-300 font-medium">Emitidas (Histórico)</span>
            ...
            <AnimatePresence>
                {showCreditModal && (
                    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
                        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ background: 'white', padding: '2.5rem', borderRadius: '20px', width: '100%', maxWidth: '500px', boxShadow: '0 20px 50px rgba(0,0,0,0.3)' }}>
                            <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                                <div style={{ backgroundColor: '#FFF8E1', padding: '15px', borderRadius: '50%', display: 'inline-block', marginBottom: '1rem' }}>
                                    <Zap color="#F57F17" size={32} />
                                </div>
                                <h2 style={{ fontWeight: '900', color: '#1A1A1A' }}>RECARGAR CRÉDITOS</h2>
                                <p style={{ color: '#666' }}>Selecciona un paquete para facturar sin límites.</p>
                            </div>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <button onClick={() => handleBuyCredits(200)} style={{ border: '2px solid #eee', padding: '1.5rem', borderRadius: '12px', textAlign: 'left', cursor: 'pointer', background: 'white', transition: 'all 0.2s', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <b style={{ fontSize: '1.1rem' }}>Paquete Inicial</b>
                                        <p style={{ fontSize: '0.8rem', color: '#27ae60', fontWeight: 'bold' }}>+2 Créditos de REGALO</p>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontWeight: '900', fontSize: '1.2rem', color: '#0ea5e9' }}>$200 MXN</div>
                                        <div style={{ fontSize: '0.7rem', color: '#999' }}>PayPal / Tarjeta</div>
                                    </div>
                                </button>

                                <button onClick={() => handleBuyCredits(500)} style={{ border: '3px solid #0ea5e9', padding: '1.5rem', borderRadius: '12px', textAlign: 'left', cursor: 'pointer', background: '#F0F7FF', position: 'relative', overflow: 'hidden' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                                            <b style={{ fontSize: '1.2rem' }}>Punto de Venta Pro</b>
                                            <p style={{ fontSize: '0.8rem', color: '#E30613', fontWeight: 'bold' }}>+10 Créditos de REGALO 🔥</p>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontWeight: '900', fontSize: '1.5rem', color: '#0ea5e9' }}>$500 MXN</div>
                                            <div style={{ fontSize: '0.7rem', color: '#999' }}>PayPal / Tarjeta</div>
                                        </div>
                                    </div>
                                </button>
                            ...
                            <button onClick={() => setShowCreditModal(false)} style={{ width: '100%', marginTop: '2rem', padding: '12px', border: 'none', background: 'none', color: '#888', fontWeight: 'bold', cursor: 'pointer' }}>
                                Tal vez más tarde
                            </button>
                        </motion.div>
                    </div>
