...
export default function OrderPool({ orders, onClaim, loading, driverLocation }: OrderPoolProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Global Driver Map */}
      <div className="w-full h-48 bg-slate-800 rounded-[32px] overflow-hidden relative shadow-lg border border-slate-700/50">
          <div className="absolute top-4 left-4 bg-slate-900/80 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/10 z-10 flex items-center gap-2 shadow-xl">
      ...
      {(loading || orders.length === 0) ? emptyState : orders.map(order => (
        <motion.div 
          key={order.id} 
        ...
        >
          <div className="flex justify-between items-start mb-3">
            ...
            <button 
              ...
              className="bg-sky-500 hover:bg-sky-400 text-white px-5 py-2.5 rounded-full text-sm font-bold flex items-center gap-1.5 transition-all active:scale-95 shadow-[0_4px_12px_rgba(14,165,233,0.4)]"
            >
              <Plus size={16} />
              Tomar
            </button>
