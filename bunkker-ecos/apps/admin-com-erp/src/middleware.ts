        ...
        if (matchedEntry) {
            ...
            if (pathname === '/dashboard') {
                if (role !== 'superadmin' && role !== 'admin') {
                    ...
                    // Si no tiene ningún rol asignado válido (ej. client o nodo nuevo), mandarlo a sala de espera
                    return NextResponse.redirect(new URL('/dashboard/waiting', request.url));
                }
                // Si es admin, lo dejamos pasar a /dashboard
            } else if (role !== 'superadmin' && role !== 'admin') {
                if (pathname === '/dashboard/waiting') {
                    // Permitir que el rol 'client' vea la sala de espera
