    ...
    useEffect(() => {
        if (isOwner) {
            setMessages([
                { 
                    role: 'assistant', 
                    content: `¡Hola Jefe! Soy Leon, tu Asistente Gerente 24/7. El sistema está operando con normalidad. En la versión PRO podré analizar a tus competidores y sugerirte precios con IA avanzada (Gemini). Por ahora, ¿qué métricas o alertas necesitas revisar hoy?` 
                }
            ]);
        } else {
            setMessages([
                { 
                    role: 'assistant', 
                    content: `¡Hola! Soy Leon, el asistente virtual de ${siteConfig.businessName || 'tu negocio'}. ¿En qué te puedo asesorar hoy respecto a nuestros productos, cotizaciones o soporte?` 
                }
            ]);
        }
    }, [isOwner, siteConfig.businessName]);
                    ...
                    >
                        <div className={styles.chatHeader} style={isOwner ? { background: '#0f172a' } : {}}>
                            <div className={styles.chatHeaderTitle}>
                                ...
                                <span className={styles.chatHeaderLabel}>{isOwner ? 'LEON - GERENTE AI (PRO)' : 'LEON - ASISTENTE VIRTUAL'}</span>
                            </div>
