...
function ProfileSetupScreen({ uid }: { uid: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success('¡Perfil configurado con éxito! La página se actualizará.', '✅ Perfil');
                ...
                unsubscribe = onSnapshot(doc(db, 'users', profile.uid), (docSnap: any) => {
                    if (docSnap.exists()) {
                        ...
                        if (dbSession && localSession && dbSession !== localSession) {
                            toast.warning('Sesión iniciada en otro dispositivo. Cerrando esta sesión por seguridad...', '⚠️ Seguridad');
                            setTimeout(() => {
                                signOut().then(() => {
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#0f111a', color: '#f8fafc' }}>
            ...
            {isMasterAdmin && (
                <aside className="liquid-glass" style={{
                    ...
                    <div style={{
                        ...
                        {!isCollapsed && (
                            <div>
                                <h2 className="mystique-scales" style={{ fontSize: '1.4rem', margin: 0, textTransform: 'uppercase' }}>
                                    {siteConfig?.businessName || 'CAMALEÓN ERP'}
                                </h2>
                                        ...
                                        return (
                                            <Link
                                                ...
                                                onClick={(e) => {
                                                    if (locked) {
                                                        e.preventDefault();
                                                        toast.info('Esta característica es exclusiva de la versión PRO. Adquiere la actualización para desbloquearla.', '🔒 Premium');
                                                    }
                                                }}
                                                style={{
                                                    display: 'flex',
            ...
            <div style={{
                ...
                width: isMasterAdmin ? `calc(100% - ${sidebarWidth})` : '100%',
