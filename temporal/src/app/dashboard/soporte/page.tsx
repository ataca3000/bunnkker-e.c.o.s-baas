...
export default function SoportePage() {
    ...
    const loadTickets = useCallback(async () => {
        ...
        try {
            ...
            // Map from SupportTicket Prisma model to UI model
            const mapped = data.map((t: any) => ({
                id: t.id,
                title: t.folio,
                description: t.message,
                priority: 'Media' as Priority, // default since schema doesn't have it
                status: t.status as TicketStatus,
                assignedTo: 'Sin asignar',
                createdBy: t.customerName,
                createdByUid: t.customerEmail,
            ...
            console.warn('[Soporte] Error fetching local tickets.', err);
            setIsOffline(true);
            setTickets([]);
        } finally {
            setLoading(false);
    ...
    useEffect(() => {
        ...
        const handleSync = (event: any) => {
            if (event.action?.includes('TICKET')) loadTickets();
        };
        ...
        try {
            const res = await fetch('/api/tickets', {
                ...
                body: JSON.stringify({
                    ...
                    customerName: profile?.displayName || 'Usuario Interno',
                    customerEmail: profile?.email || 'admin@bunkker.com'
                })
            });
            const result = await res.json();
            if (result.success) {
                if (typeof window !== 'undefined' && (window as any).__inventorySocket) {
                    (window as any).__inventorySocket.emit('sync_db_event', { action: 'TICKET_ADDED' });
                }
                loadTickets();
                setShowForm(false);
            } else {
            ...
            toast.error('Error al crear el ticket. Verifica tu conexión.');
        } finally {
            setSaving(false);
        }
    }, [profile, loadTickets]);
    ...
    const handleStatusChange = useCallback(async (ticket: SupportTicket) => {
        ...
        try {
            ...
            if (res.ok) {
                if (typeof window !== 'undefined' && (window as any).__inventorySocket) {
                    (window as any).__inventorySocket.emit('sync_db_event', { action: 'TICKET_UPDATED' });
                }
                loadTickets();
            }
        } catch (err) {
