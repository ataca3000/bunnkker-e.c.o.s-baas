...
export default function TeamManagement() {
    ...
    const handleSaveRole = async (userId: string) => {
        ...
        if (isReadOnly) { toast.warning('No se pueden guardar cambios en Modo Demo.', '🔒 Demo'); setEditingUserId(null); return; }
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.error(data.error, 'Error de rol');
            }
    ...
    const handleDeleteUser = async (userId: string) => {
        ...
        if (isReadOnly) { toast.warning('Operación no permitida en Modo Demo.', '🔒 Demo'); return; }
        ...
        } catch { toast.error('Error al borrar el usuario'); }
    };

    const handleCreateUser = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newName || !newPin || !newRole || isReadOnly) return;
        if (newPin.length < 4) { toast.warning('El PIN debe tener al menos 4 dígitos.'); return; }
        ...
        try {
            ...
            if (data.success) {
                ...
                setNewName('');
                setNewPin('');
                toast.success('Usuario creado exitosamente.', '✅ Equipo');
            } else {
                toast.error('Error al crear usuario: ' + data.error, 'Error');
            }
        } catch (err: any) { 
    ...
    const handleOpenQr = async (user: UserData) => {
        ...
        try {
            ...
            if (data.success && typeof window !== 'undefined') {
                ...
                toast.error('No se pudo generar la llave mágica.');
                setQrUser(null);
            }
        } catch {
            toast.error('Error de conexión al generar la llave.');
            setQrUser(null);
        }
    };
    ...
    const handleResetDevice = async (userId: string) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                toast.success('Dispositivo desvinculado con éxito.', '✅ Dispositivo');
            } else {
                toast.error('Error al desvincular: ' + data.error);
            }
        } catch (err: any) {
            toast.error('Error de conexión con el servidor.');
        }
    ...
    return (
        <div className="relative min-h-[100dvh] bg-slate-950 text-white p-6 md:p-10 font-sans">
            <div className="fixed inset-0 z-0 pointer-events-none">
                <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-purple-900/40 blur-[120px] rounded-full"></div>
                <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-sky-900/30 blur-[120px] rounded-full"></div>
            </div>
            
            <div className="relative z-10 max-w-6xl mx-auto">

                {/* Header */}
                <header className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-10 bg-slate-900/60 backdrop-blur-md p-6 rounded-sm shadow-[0_0_20px_rgba(168,85,247,0.1)] border border-purple-500/30">
                    <div className="flex items-center gap-5">
                        <div className="bg-purple-900/40 p-4 rounded-sm text-purple-400 border border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.3)]">
                            <ShieldCheck size={36} />
                        </div>
                        <div>
                            <h1 className="text-3xl font-[950] text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-sky-400 to-emerald-400 uppercase tracking-tight drop-shadow-md">
                                Equipo y Roles
                            </h1>
                            <p className="text-purple-300 text-sm font-bold tracking-widest uppercase mt-1">Gestión de empleados local</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 bg-slate-800/80 border border-purple-500/20 px-4 py-2 rounded-sm text-sm text-purple-200 shadow-inner">
                        <Users size={16} className="text-sky-400" />
                        <span><b className="text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.8)]">{users.length}</b> miembros</span>
                    </div>
                </header>
